function [imageOut,template1] = normcorre_doppler_human_preProcessing_ka(imageIn, refLen, bwd, template, verbose)
gcp;
...
options_rigid = NoRMCorreSetParms('d1',size(Y,1),'d2',size(Y,2),'bin_width', bwd,'max_shift', 10,'us_fac',50,'init_batch', refLen);
options_nonrigid = NoRMCorreSetParms('d1',size(Y,1),'d2',size(Y,2),'grid_size',[16,16],'mot_uf',4,'bin_width',120,'max_shift',10,'max_dev',3,...
    'us_fac',50,'init_batch',refLen);

%% perform motion correction
if useTemplate
    tic; [M1,shifts1,template1,options_rigid] = normcorre(Y,options_rigid,template); toc %#ok<*ASGLU>
else
    tic; [M1,shifts1,template1,options_rigid] = normcorre(Y,options_rigid); toc %#ok<*ASGLU>
end

% if useTemplate
%     tic; [M1,shifts1,template1,options_nonrigid] = normcorre(Y,options_nonrigid,template); toc %#ok<*ASGLU>
% else
%     tic; [M1,shifts1,template1,options_nonrigid] = normcorre(Y,options_nonrigid); toc %#ok<*ASGLU>
% end
...
if verbose
    ...
    figure;
        ...
        ax2 = subplot(2,3,2); imagesc(mM1p,[0,1]);  axis equal; axis tight; axis off; title('mean rigid corrected','fontsize',14,'fontweight','bold')
        ax3 = subplot(2,3,3); imagesc(mM2p,[0,1]); axis equal; axis tight; axis off; title('mean non-rigid corrected','fontsize',14,'fontweight','bold')
    ...
    figure;
        ax1 = subplot(311); plot(1:T,cY,1:T,cM1,1:T,cM2); legend('raw data','rigid','non-rigid'); 
