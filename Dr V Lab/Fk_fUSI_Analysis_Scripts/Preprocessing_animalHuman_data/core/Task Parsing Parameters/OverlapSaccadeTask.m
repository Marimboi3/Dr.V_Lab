function [t, parameterTable, parameter_counter, cnt_attempt] = OverlapSaccadeTask(CurrentSession, ...
    parameterTable, ...
    parameter_counter, ...
    cnt_attempt, ...
    session)
%% Parses behavioral data for overlap saccade task
% Has no memory period
% Inputs: 
...
for trial = 1:nTrials
    ...
    if nStates >= 4 % attempted (monkey got to cue) (4 because initialfixation, fixationhold, cue, success or fail iti, plus other states)
        ...
        t(cnt_attempt).success           = nStates > 6; % Successful trial if it makes to at least 7 states (fixation acquire, fixation hold, cue, target acquire, target hold, reward, and iti)
